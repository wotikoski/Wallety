...
export async function GET(req: NextRequest) {
  try {
    ...
    // Fixed = generated by a recurrence OR manually marked as fixed
    const fixedTotal = txns
      .filter((t) => t.recurrenceGroupId !== null || t.isFixed)
      .reduce((acc, t) => acc + parseFloat(t.value), 0);
    const variableTotal = grandTotal - fixedTotal;

    return NextResponse.json({ items, grandTotal, groupBy, fixedTotal, variableTotal });
  } catch (e) {
